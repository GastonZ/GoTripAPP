...
const Options = () => {
    const [currentUserBlind, setCurrentUserBlind] = useState(localStorage.getItem('isNoVidente') === "true");

    useEffect(() => {
        const handleStorageChange = () => {
            setCurrentUserBlind(localStorage.getItem('isNoVidente') === "true");
        };
    ...
    const buttonArray = [
        { text: 'Realizar plan de viaje', icon: <PencilIcon className='w-10 h-10' />, href: '/calendario' },
        { text: 'Puntos turisticos & eventos', icon: <MapIcon className='w-10 h-10' />, href: '/puntos-turisticos' },
        { text: 'Mis viajes', icon: <HeartIcon className='w-10 h-10' />, href: '/misviajes' },
        { text: 'Puntos de carga', icon: <Battery100Icon className='w-10 h-10' />, href: '/puntos-de-carga' },
        { text: 'Consultar a chatbot', icon: <ChatBubbleBottomCenterIcon className='w-10 h-10' />, href: '/chatbot' },
    ];

    const buttonArrayBlind = [
        { text: 'Guia Turistica', icon: <ChatBubbleBottomCenterIcon className='w-10 h-10' />, href: '/guia-turistica' },
    ];
